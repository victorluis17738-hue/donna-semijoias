...
async function preparePreview(buf, filename, userEmail) {
  const parsed = await parser.parseBuffer(buf, filename);
  ...
  for (const c of candidates) {
    // Com SKU: casa SOMENTE por SKU. Se o SKU nao existe, e' produto NOVO (o SKU e'
    // a identidade — nao "rouba" um produto existente so' porque o nome parece igual).
    // Sem SKU no arquivo: ai sim casa por nome (protege o catalogo antigo sem SKU).
    let match = null;
    if (c.mapped.sku) match = bySku.get(c.mapped.sku.trim().toLowerCase());
    else if (c.name) match = byName.get(mapper.normalizeHeader(c.name));

    if (match) {
      c.action = 'update';
      c.existingId = match.id;
      c.existingSku = match.sku || null;
      willUpdate += 1;
      tagged.push(c);
    } else {
  ...
  // SKUs "presentes" no catalogo = os do arquivo + os SKUs dos produtos que
  // casaram por nome (senao "desativar ausentes" apagaria produto recem-atualizado).
  const importedSkus = [];
  for (const c of tagged) {
    if (c.mapped.sku) importedSkus.push(c.mapped.sku.trim().toLowerCase());
    if (c.existingSku) importedSkus.push(String(c.existingSku).trim().toLowerCase());
  }
...
function startCommit(uploadId, options, userEmail) {
  ...
  uploads.delete(uploadId); // consome a sessao: impede confirmar o mesmo arquivo duas vezes
...
async function runJob(job, up, deactivateMissing, userEmail) {
  ...
  // Um UPDATE por produto existente (evita id repetido no mesmo lote -> clobber).
  const updateMap = new Map();
  for (const c of up.tagged) if (c.action === 'update') updateMap.set(c.existingId, c);
...
async function insertBatch(batch) {
  ...
  // DO NOTHING: se o SKU ja existir (corrida / confirmar duas vezes), NAO sobrescreve
  // o produto existente com os valores-padrao do insert. Atualizacoes de verdade
  // passam pelo updateBatch (produtos que o preview classificou como "atualizar").
...
function errorsToCsv(errors) {
  ...
  const escape = (v) => {
    ...
    // Anti "formula injection": Excel executa celulas que comecam com = + - @ etc.
    if (/^[=+\-@\t\r]/.test(s)) s = `'${s}`;
    return `"${s.replace(/"/g, '""')}"`;
  };
