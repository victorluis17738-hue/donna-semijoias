...
const FIELD_DEFS = [
  ...
  { field: 'image_url', syn: ['imagem', 'url da imagem', 'url imagem', 'foto', 'fotos', 'link da imagem', 'imagem url', 'link da foto', 'link foto', 'foto url', 'url foto'] },
...
// Converte "R$ 1.299,90" / "1299.90" / "129,90" / "1.290" em numero. NaN se invalido.
function parseMoney(value) {
  ...
  if (hasComma) {
    // Virgula presente = decimal brasileiro; pontos sao separador de milhar.
    s = s.replace(/\./g, '').replace(',', '.');
  } else if (hasDot) {
    // So' pontos: "1.290" ou "1.234.567" e' milhar; "12.99" e' decimal.
...
// Estoque: inteiro entre 0 e ~2 bilhoes (cabe no INTEGER do Postgres, sem abortar
// o lote). Aceita "10", "10,00", "10.00", "1.000". Retorna null quando vazio.
function parseStock(value) {
  if (value == null || value === '') return null;
  const n = parseMoney(value);
  if (!Number.isFinite(n)) return null;
  return Math.min(2000000000, Math.max(0, Math.trunc(n)));
}
...
function mapRow(cells, mapping) {
  ...
  return {
    ...
    cost_price: Number.isFinite(cost_price) && cost_price >= 0 ? cost_price : null,
    stock,
    images,
    details
  };
